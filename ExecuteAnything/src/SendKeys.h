...
public:
	// SendKey string. SHIFT=+ CTRL=^  ALT=%  IE: ctrl-F4 = ^{F4}
	bool SendString(LPCTSTR KeysString, bool Wait = false);
	...
	SendKeys(HWND wnd = NULL);
	SendKeys(LPCTSTR WindowTitle, LPCTSTR WindowClass, LPCTSTR KeysString = NULL, bool Wait = false);
	SendKeys(LPCTSTR KeysString, HWND wnd = NULL, bool Wait = false);
};
