...
#include <windows.h>
/* 
	Code refractor by David Maisonave (axter.com)
	Refractor original nppexec_msgs.h code to address the following:
	Replace #define to enum so as to remove global namespace poluting.
	To simplify the code:
		Replace all versions of NPE_*_OK and NPE_*_FAILED to NPE_RESULT_OK and NPE_RESULT_FAILED.  When using standard programming practice the same status ID is used for all interfaces in an API set.
		Made NpeExecuteCollateralParam, NpeExecuteParam2, and NpeExecuteQueuedParam have the same type, so that the same data can be optionally used on multiple API's.
	For original code and usage instructions, look at the commented section at the end of this file.
*/
struct tPluginResultStruct {
...
struct NpeNppExecParam2 {
	const TCHAR* szScriptName;      // name of existing script/file
	const TCHAR* szScriptArguments; // arguments, can be NULL (i.e. none)
	const TCHAR* szID;              // the ID of this message (e.g. to be verified on NPEN_RESULT)
	DWORD        dwResult;          // [out] NPE_RESULT_OK - OK; otherwise failed
};

struct NpeExecuteBase {
	inline NpeExecuteBase() :szScriptBody(0), szID(0), dwResult(0) {}
	const TCHAR* szScriptBody; // text of the script (i.e. set of commands)
	const TCHAR* szID;         // the ID of this message (e.g. to be verified on NPEN_RESULT)
	DWORD        dwResult;     // [out] NPE_RESULT_OK - OK; otherwise failed
};

using NpeExecuteCollateralParam = NpeExecuteBase;
using NpeExecuteParam2 = NpeExecuteBase;
using NpeExecuteQueuedParam = NpeExecuteBase;

struct NpeGetScriptNamesParam {
	TCHAR* pScriptNames; // [out] returned pointer to script names
	DWORD  dwResult;     // [out] NPE_RESULT_OK - OK; otherwise failed
};

struct NpeGetScriptByNameParam {
	const TCHAR* szScriptName;  // script name
	TCHAR*       pScriptBody;   // [out] returned pointer to the script body
	DWORD        dwResult;      // [out] NPE_RESULT_OK - OK; otherwise failed
};

// Using enum over #define because enum's don't polute the global namespace like #define
enum NPE_MSG_ {
	NPE_STATEDISABLED = 0
	, NPE_STATEREADY = 0x01
	, NPE_STATEBUSY = 0x10
	, NPE_MAXVERSTR = 32
	, NPEM_EXECUTE = 0x0101// message (NppExec version >= 0x02F5)
	, NPEM_NPPEXEC = 0x0102// message (NppExec version >= 0x02F5)
	, NPEN_RESULT = 0x0109// notification (NppExec version >= 0x02F5)
	, NPEM_EXECUTE2 = 0x0151// message (NppExec version >= 0x06A0)
	, NPEM_NPPEXEC2 = 0x0152// message (NppExec version >= 0x06A0)
	, NPEM_EXECUTE_COLLATERAL = 0x0153// message (NppExec version >= 0x06A0)
	, NPEM_EXECUTE_QUEUED = 0x0154// message (NppExec version >= 0x06A0)
	, NPEM_GETVERDWORD = 0x0201// message
	, NPEM_GETVERSTR = 0x0202// message (NppExec version >= 0x02F5)
	, NPEM_GETSTATE = 0x0301// message (NppExec version >= 0x02F5)
	, NPEM_PRINT = 0x0401// message (NppExec version >= 0x02F5)
	, NPEM_FREEPTR = 0x0500// message (NppExec version >= 0x06C1)
	, NPEM_GETSCRIPTNAMES = 0x0501// message (NppExec version >= 0x06C1)
	, NPEM_GETSCRIPTBYNAME = 0x0502// message (NppExec version >= 0x06C1)
	, NPEM_INTERNAL = 0xFF01// internal messages, don't use it!
	, NPEM_SUSPENDEDACTION = (NPEM_INTERNAL + 1) // <-- obsolete
	, NPE_STATECHILDPROC = (NPE_STATEBUSY | 0x20)
};

enum NPE_RESULT_STATUS {
	NPE_RESULT_DISABLED = NPE_STATEDISABLED,
	NPE_RESULT_OK = NPE_STATEREADY,
	NPE_RESULT_FAILED = NPE_STATEBUSY,
	NPE_RESULT_ABORTED = 0x05
};
...
// #define  NPEM_GETVERDWORD       0x0201  // message // *** {NPEM_GETVERDWORD Replaced with enum} ***
  /*
  ...
  */
  ...
  // #define  NPEM_GETVERSTR         0x0202  // message (NppExec version >= 0x02F5) // *** {NPEM_GETVERSTR Replaced with enum} ***
	// #define  NPE_MAXVERSTR            32 // *** {NPE_MAXVERSTR Replaced with enum} ***
	/*
	...
	*/
	...
	// #define    NPEM_GETSTATE          0x0301	// message (NppExec version >= 0x02F5) // *** {NPEM_GETSTATE Replaced with enum} ***
	  // #define  NPE_STATEDISABLED      0x00 // *** {NPE_STATEDISABLED Replaced with enum} ***
	  // #define  NPE_STATEREADY         0x01 // *** {NPE_STATEREADY Replaced with enum} ***
	  // #define  NPE_STATEBUSY          0x10 // *** {NPE_STATEBUSY Replaced with enum} ***
	  // #define  NPE_STATECHILDPROC     (NPE_STATEBUSY | 0x20) // *** {NPE_STATECHILDPROC Replaced with enum} ***
	  /*
	  ...
	  */
	  ...
	  // #define  NPEM_PRINT             0x0401  // message (NppExec version >= 0x02F5) // *** {NPEM_PRINT Replaced with enum} ***
		/*
		...
		*/
		...
		// #define  NPEM_EXECUTE           0x0101  // message (NppExec version >= 0x02F5) // *** {NPEM_EXECUTE Replaced with enum} ***
		  // #define  NPE_EXECUTE_OK       NPE_STATEREADY // *** {NPE_EXECUTE_OK Replaced with NPE_RESULT_FAILED} ***
		  // #define  NPE_EXECUTE_FAILED   NPE_STATEBUSY // *** {NPE_EXECUTE_FAILED Replaced with NPE_RESULT_FAILED} ***
...
// #define  NPEM_NPPEXEC           0x0102  // message (NppExec version >= 0x02F5) // *** {NPEM_NPPEXEC Replaced with enum} ***
  // #define  NPE_NPPEXEC_OK       NPE_STATEREADY // *** {NPE_NPPEXEC_OK Replaced with NPE_RESULT_OK} ***
  // #define  NPE_NPPEXEC_FAILED   NPE_STATEBUSY // *** {NPE_NPPEXEC_FAILED Replaced with NPE_RESULT_FAILED} ***
...
// #define  NPEM_EXECUTE2           0x0151  // message (NppExec version >= 0x06A0) // *** {NPEM_EXECUTE2 Replaced with enum} ***
  // #define  NPE_EXECUTE2_OK       NPE_STATEREADY // *** {NPE_EXECUTE2_OK Replaced with NPE_RESULT_OK} ***
  // #define  NPE_EXECUTE2_FAILED   NPE_STATEBUSY // *** {NPE_EXECUTE2_FAILED Replaced with NPE_RESULT_FAILED} ***
...
// #define  NPEM_NPPEXEC2           0x0152  // message (NppExec version >= 0x06A0) // *** {NPEM_NPPEXEC2 Replaced with enum} ***
  // #define  NPE_NPPEXEC2_OK       NPE_STATEREADY // *** {NPE_NPPEXEC2_OK Replaced with NPE_RESULT_OK} ***
  // #define  NPE_NPPEXEC2_FAILED   NPE_STATEBUSY // *** {NPE_NPPEXEC2_FAILED Replaced with NPE_RESULT_FAILED} ***
...
// #define  NPEM_EXECUTE_COLLATERAL           0x0153  // message (NppExec version >= 0x06A0) // *** {NPEM_EXECUTE_COLLATERAL Replaced with enum} ***
  // #define  NPE_EXECUTE_COLLATERAL_OK       NPE_STATEREADY // *** {NPE_EXECUTE_COLLATERAL_OK Replaced with NPE_RESULT_OK} ***
  // #define  NPE_EXECUTE_COLLATERAL_FAILED   NPE_STATEBUSY // *** {NPE_EXECUTE_COLLATERAL_FAILED Replaced with NPE_RESULT_FAILED} ***
...
if ( necp.dwResult == NPE_RESULT_OK )
{
  // OK, the script is being executed
  // you'll get NPEN_RESULT when the script is finished
}
...
// #define  NPEM_EXECUTE_QUEUED           0x0154  // message (NppExec version >= 0x06A0) // *** {NPEM_EXECUTE_QUEUED Replaced with enum} ***
  // #define  NPE_EXECUTE_QUEUED_OK       NPE_STATEREADY // *** {NPE_EXECUTE_QUEUED_OK Replaced with NPE_RESULT_OK} ***
  // #define  NPE_EXECUTE_QUEUED_FAILED   NPE_STATEBUSY // *** {NPE_EXECUTE_QUEUED_FAILED Replaced with NPE_RESULT_FAILED} ***
...
if ( neqp.dwResult == NPE_RESULT_OK )
{
...
// #define  NPEN_RESULT           0x0109  // notification (NppExec version >= 0x02F5) // *** {NPEN_RESULT Replaced with enum} ***
  // #define  NPE_RESULT_OK       NPE_STATEREADY // *** {NPE_RESULT_OK Replaced with enum} ***
  // #define  NPE_RESULT_FAILED   NPE_STATEBUSY // *** {NPE_RESULT_FAILED Replaced with enum} ***
  // #define  NPE_RESULT_ABORTED  0x05 // *** {NPE_RESULT_ABORTED Replaced with enum} ***
  /*
  ...
  */
  ...
  // #define  NPEM_FREEPTR                   0x0500  // message (NppExec version >= 0x06C1) // *** {NPEM_FREEPTR Replaced with enum} ***
	/*
	...
	*/
	...
	// #define  NPEM_GETSCRIPTNAMES            0x0501  // message (NppExec version >= 0x06C1) // *** {NPEM_GETSCRIPTNAMES Replaced with enum} ***
	  // #define  NPE_GETSCRIPTNAMES_OK        NPE_STATEREADY // *** {NPE_GETSCRIPTNAMES_OK Replaced with NPE_RESULT_OK} ***
	  // #define  NPE_GETSCRIPTNAMES_FAILED    NPE_STATEBUSY // *** {NPE_GETSCRIPTNAMES_FAILED Replaced with NPE_RESULT_FAILED} ***
...
// #define NPEM_GETSCRIPTBYNAME            0x0502  // message (NppExec version >= 0x06C1) // *** {NPEM_GETSCRIPTBYNAME Replaced with enum} ***
  // #define  NPE_GETSCRIPTBYNAME_OK       NPE_STATEREADY // *** {NPE_GETSCRIPTBYNAME_OK Replaced with NPE_RESULT_OK} ***
  // #define  NPE_GETSCRIPTBYNAME_FAILED   NPE_STATEBUSY // *** {NPE_GETSCRIPTBYNAME_FAILED Replaced with NPE_RESULT_FAILED} ***
...
// #define  NPEM_INTERNAL         0xFF01  // internal messages, don't use it! // *** {NPEM_INTERNAL Replaced with enum} ***
// #define  NPEM_SUSPENDEDACTION  (NPEM_INTERNAL + 1) // <-- obsolete // *** {NPEM_SUSPENDEDACTION Replaced with enum} ***
